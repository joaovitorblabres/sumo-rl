...
if __name__ == '__main__':
    ...
    out_csv = 'outputs/MOD_PQL_TESTE_ND/gamma{}_eps{}_decay{}/{}/{}/'.format(args.gamma, args.epsilon, args.decay, experiment_time[0], experiment_time[1])
    debug = args.debug
    ...
    for run in range(1, args.runs+1):
        ...
        ql_agents = {ts: PQLAgent(starting_state=env.encode(initial_states[ts], ts),
                                 ...
                                 exploration_strategy=MOSelection(initial_epsilon=args.epsilon, min_epsilon=args.min_epsilon, decay=args.decay, ref=np.asarray([-100, -100]))) for ts in env.ts_ids}
            ...
            if args.fixed:
                ...
                while not done['__all__']:
                    ...
                    for ts in ql_agents.keys():
                        ...
                        density[ts+"s_a_ns_r"].append([states[ts], actions[ts], next_state, r[ts], env.traffic_signals[ts].flow, sum(env.traffic_signals[ts].get_waiting_time_per_lane())/max(1,env.traffic_signals[ts].last_cars), env.traffic_signals[ts].last_cars])
