...
if __name__ == '__main__':
    ...
    for run in range(1, args.runs+1):
        ...
        ql_agents = {ts: PQLAgent(starting_state=env.encode(initial_states[ts], ts),
                                 ...
                                 action_space=env.action_spaces(ts),
                                 gamma=args.gamma,
                                 ref_point=np.asarray([-8, 0]),
                                 exploration_strategy=MOSelection(initial_epsilon=args.epsilon, min_epsilon=args.min_epsilon, decay=args.decay, ref=np.asarray([-8, 0]), algType=args.algType)) for ts in env.ts_ids}
