...
from sumo_rl.agents import modPQLAgent
...
if __name__ == '__main__':
    ...
    prs.add_argument("-gt", dest="gt", type=float, default=0.5, required=False, help="Gamma discount rate.\n")
    ...
    prs.add_argument("-collect", action="store_true", default=False, help="Debug spent time in each function.\n")
    ...
    for run in range(1, args.runs+1):
        ...
        groups = {0: {'agents': ['E2', 'H5', 'E8', 'B5']}, 1: {'agents': ['F3', 'G6', 'D7', 'C4']}, 2: {'agents': ['E3', 'G5', 'E7', 'C5']}}
        ql_agents = {ts: modPQLAgent(starting_state=env.encode(initial_states[ts], ts),
                                 state_space=env.observation_spaces(ts),
                                 action_space=env.action_spaces(ts),
                                 gamma=args.gamma,
                                 gt=args.gt,
                                 ref_point=np.asarray([-100, -100]),
                                 exploration_strategy=MOSelection(initial_epsilon=args.epsilon, min_epsilon=args.min_epsilon, decay=args.decay, ref=np.asarray([-100, -100]), algType=args.algType)) for ts in env.ts_ids}

        twt = []
        volumes = []
        for group in groups.keys():
            for agent in groups[group]['agents']:
                ql_agents[agent].grouped = 1
        for ep in range(1, args.eps+1):
            ...
            if args.fixed:
                ...
                while not done['__all__']:
                    ...
                    for group in groups.keys():
                        for agent in groups[group]['agents']:
                            copia = copy.deepcopy(groups[group]['agents'])
                            copia.remove(agent)
                            for others in copia:
                                ql_agents[agent].avg_others += ql_agents[others].avg_r
                            ql_agents[agent].avg_others /= len(copia)
