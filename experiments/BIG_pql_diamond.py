...
from sumo_rl.agents import bigPQLAgent
...
if __name__ == '__main__':
    ...
    debug = args.debug
    out_csv = 'outputs/BIG_{}{}{}TESTE/gamma{}_eps{}_decay{}/{}/{}/'.format(['PO_' if args.algType else 'HV_'][0], ['FIXED_' if args.fixed else ''][0], ['DEBUG_' if args.debug else ''][0], args.gamma, args.epsilon, args.decay, experiment_time[0], experiment_time[1])
    ...
    for run in range(1, args.runs+1):
        ...
        ql_agents = {ts: bigPQLAgent(starting_state=env.encode(initial_states[ts], ts),
                                 state_space=env.observation_spaces(ts),
                                 action_space=env.action_spaces(ts),
                                 gamma=args.gamma,
                                 ref_point=np.asarray([-100, -100, -100, -100, -100, -100, -100, -100, -100, -100, -100]),
                                 exploration_strategy=MOSelection(initial_epsilon=args.epsilon, min_epsilon=args.min_epsilon, decay=args.decay, ref=np.asarray([-100, -100, -100, -100, -100, -100, -100, -100, -100, -100, -100]), algType=args.algType)) for ts in env.ts_ids}
        ...
        for ep in range(1, args.eps+1):
            ...
            if args.fixed:
                ...
                while not done['__all__']:
                    ...
                    s, r, done, _ = env.step(action=actions, testing = True)
